...
**🆕 Nouvelle dette relevée au re-audit du 05/08/2026 :**
- 🟡 **`7-LA — Lire liste_attente` sans `alwaysOutputData`** — lookup `telephone` + `statut=en_attente`, atteint sur la branche annulation où 0 résultat est légitime → branche morte silencieuse sans `aod`. Fix : activer `Always Output Data`. *(Seule lecture Sheets « 0-résultat légitime » à ne pas avoir `aod` ; les 7 autres l'ont déjà.)*
- 🟡 **34 résidus `.item` (hors `7-LA — Écrire patients_rdv`, corrigé)** — répartis sur 18 nœuds ; classés à l'audit : 25 sûrs (source `Set`/`Code` amont, pairing 1:1) + 9 « à risque » (source change compte/ordre). Aucun n'est un bug en mono-patient. Seul `7-LA — Marquer LA traitée` → `$('7-LA — Lire liste_attente').item` (lookup Sheets) mérite un `.first()` par précaution. Priorité basse, à durcir avant tout batch.
...
*Archive mise à jour le **05/08/2026**. Rappel synthèse v17 → v28 : cabinet Badrour, nom médecin bilingue (`projeter()`), branches ESCAPE / choix RDV existant / RDV expiré, `7-ACR-LIRE`, bug T2.8 clôturé. **Nouveau au 05/08 : les 3 dettes d'audit v28 (CRON chaque minute, `nb_annulations` sans fallback, `.item` résiduel dans `7-LA — Écrire patients_rdv`) sont vérifiées RÉSOLUES dans `WhatsApp_Appointment_Automation.json`** et archivées en PARTIE A. `name` interne renommé. Une dette mineure ouverte : `7-LA — Lire liste_attente` sans `alwaysOutputData` (+ résidus `.item` d'hygiène, non bloquants). Note test : `Plan_Test_Workflow1_v14.xlsx` couvre encore la matrice v14–v17 ; les sous-flux v28 (ESCAPE, choix RDV existant, RDV expiré) + le chemin NON de l'inscription LA restent à ajouter au plan de test.*
